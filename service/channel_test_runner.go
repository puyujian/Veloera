...
// InitChannelBatchTestRunner 初始化批量模型测试调度器
func InitChannelBatchTestRunner() {
    channelTestRunnerOnce.Do(func() {
        channelTestRunner = &ChannelTestJobRunner{
            running: make(map[int64]*channelTestJobRuntime),
        }
        // 清理可能遗留的 RUNNING 状态任务
...
func getChannelTestRunner() *ChannelTestJobRunner {
    if channelTestRunner == nil {
        InitChannelBatchTestRunner()
    }
    return channelTestRunner
}

// SubmitChannelTestJob 将任务提交到后台执行
func SubmitChannelTestJob(jobID int64) error {
    runner := getChannelTestRunner()
    runner.mu.Lock()
...
// CancelChannelTestJob 取消正在执行的任务
func CancelChannelTestJob(jobID int64) error {
    runner := getChannelTestRunner()
    runner.mu.Lock()
